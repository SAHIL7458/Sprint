...
-- US_SQL_001: First 50% Logged-in Customers
SELECT customer_id
FROM (
    SELECT customer_id,
...
-- US_SQL_003: Distinct Products
SELECT DISTINCT product_name, price FROM Product;

-- US_SQL_003: Customer–Product Mapping
SELECT customer_id, product_name, product_id FROM Product ORDER BY customer_id;

-- US_SQL_004: All Transactions (Descending)
SELECT * FROM TransactionTable ORDER BY total_amount DESC;

-- US_SQL_004: Second Highest Transaction
SELECT * FROM (
...
-- US_SQL_005: Successful Orders Report
SELECT 
    r.customer_id,
    r.customer_name,
    r.email,
